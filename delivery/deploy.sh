...
docker run --rm -d --name team-b-html -p $1:8080 -v $(pwd)/frontend/build/:/usr/share/nginx/html -v $(pwd)/config/:/etc/nginx/conf.d nginx /bin/bash -c "systemctl restart nginx"
